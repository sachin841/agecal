...
let template;
try {
    template = fs.readFileSync("yard-calculator-gravel.html", "utf8");
} catch (err) {
    console.error("Error: yard-calculator-gravel.html not found. Please create it first.");
    process.exit(1);
}
...
if (!fs.existsSync(calculatorsDir)) {
    fs.mkdirSync(calculatorsDir);
}

// Generate the pages
variables.forEach(varName => {
    // Create a URL-friendly filename
    const filename = "yard-calculator-" + varName.toLowerCase().replace(/ /g, "-").replace(/\(|\)|"/g, "") + ".html";
    const filepath = path.join(calculatorsDir, filename);

    // Replace placeholders in the template
    let content = template.replace(/Gravel/g, varName);
    content = content.replace(/gravel/g, varName.toLowerCase());
    content = content.replace(/Calculate the amount of gravel needed for your yard project./g, `Calculate ${varName.toLowerCase()} for your yard project.`);
